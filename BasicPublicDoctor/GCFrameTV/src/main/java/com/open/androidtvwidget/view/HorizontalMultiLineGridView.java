 ...
 * 
 */
public class HorizontalMultiLineGridView extends RelativeLayout {
	...
	public HorizontalMultiLineGridView(Context context, AttributeSet attrs,
			int defStyle) {
		super(context, attrs, defStyle);
	}

	public HorizontalMultiLineGridView(Context context) {
		super(context);
	}
...
//		}
	...
	private void returnCover(View item) {
		...
		if (scalable) {
			Log.i("key", "setSelection---------" + selectIndex);
//			scaleToNormal(item);
			item.setSelected(false);
			cursor.animate().scaleX(1.0f).scaleY(1.0f)
			.setDuration(250)
			.start();
		}
	}
...
//				scaleToLarge(item);
				item.setSelected(true);
				cursor.animate().scaleX(1.05f).scaleY(1.05f)
...
//			}
	...
	public void setSelection(int direction, final int position) {
		...
		if (position != selectIndex) {
			...
			if (position != -1) {
				...
				// @Override
				// public void run() {
				// if(findViewById(itemIds.keyAt(position))!=null){
				// }
				// }
				//
				// }, delay);
				...
				selectCol = position / 2;
				if (itemIds.size() > position) {
	...
	private void scroll(int direction, int position) {
		...
		if (next != null) {
			// 向下到达最后一完整行时,可滚动; 向上到达最上一行完整行时,可滚动
			Log.i("index", "position:"+ position+"selectIndex:"+selectIndex);
			if (position > selectIndex) {
				if ((next.getLeft() - mScroller.getFinalX()) >= (rowWidth * (screenMaxColumns - 1))
						+ paddingLeft) {
					flag = true;
