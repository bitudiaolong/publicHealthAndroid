...
import com.open.androidtvwidget.view.component.RecycleBin;
import com.open.androidtvwidget.view.component.TvBaseAdapter;
import com.open.androidtvwidget.view.component.TvUtil;
 ...
 * 
 */
public class TvHorizontalListView extends RelativeLayout {
	...
	public TvHorizontalListView(Context context, AttributeSet attrs) {
		...
		this.scrollDuration = custom.getInteger(
				R.styleable.TvHorizontalGridView_scrollDuration, 100);
	...
	private void initItems() {
		...
		int initLength = Math.min(adapter.getCount(), 100);
	...
	public void setSelection(final int direction, final int position) {
		...
		if (position != selectIndex) {
			if (selectIndex != -1) {
				Log.i("ItemLayout",
						"---------------dispatchSetSelected---------"
								+ selectIndex);
				returnCover(findViewById(itemIds.keyAt(selectIndex)));
				// 选中事件
				if (onItemSelectListener != null) {
					onItemSelectListener.onItemDisSelect(
							findViewById(itemIds.keyAt(selectIndex)),
							selectIndex);
				}
			}
			if (position != -1) {
				...
				if (direction == View.FOCUS_LEFT
					...
					if (onItemSelectListener != null) {
						onItemSelectListener
								.onItemSelect(
										findViewById(itemIds.keyAt(position)),
										position);
					}
				}
				Log.i("ItemLayout",
						"---------------dispatchSetSelected---------"
								+ position);
				moveCover(findViewById(itemIds.keyAt(position)));
	...
	// Log.i("Help", "dispatchKeyEventPreIme-----" + event.getKeyCode());
	...
	// focusIsOut = false;
	// } else {
	// Log.i("Help", "dispatchKeyEventPreIme--focusIsOut---");
	// parentLayout = true;
	// focusIsOut = true;
	...
	private void scroll(int direction, int position) {
		boolean flag = false;
		View next = findViewById(itemIds.keyAt(position));
		// switch (direction) {
		// case View.FOCUS_RIGHT:
		// // 获得目标焦点控件
		// if (position < itemIds.size() - 1) {
		// next = findViewById(itemIds.keyAt(position + 1));
		// }
		// break;
		...
		if (l == mScroller.getFinalX()) {
			if (l > oldl) {
				...
				Log.i(VIEW_LOG_TAG, "添加更多条目=" + colCount + "--" + selectCol
						+ "--" + screenMaxColumns);
				if ((colCount - selectIndex) < screenMaxColumns) {
	...
	private void scrollByRow(final int direction) {
		if (selectIndex < 0 || selectIndex > colCount - 1) {
			return;
		}

		View itemView = findViewById(itemIds.keyAt(selectIndex));
		...
		if (direction == View.FOCUS_LEFT) {
			...
			// scrollDuration);
			Log.i("ItemLayout", "FOCUS_LEFT------scrollByRow-----"
					+ selectIndex);
			mScroller.startScroll(mScroller.getFinalX(), 0, -rowWidth, 0, 100);
			...
			// itemView.getLeft(),
			// 0, scrollDuration);
			Log.i("ItemLayout", "FOCUS_RIGHT-------scrollByRow----"
					+ selectIndex);
			mScroller.startScroll(mScroller.getFinalX(), 0, rowWidth, 0, 100);
		}
	...
	private void scaleToLarge(final View item) {
		...
		anim.addUpdateListener(new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				item.setScaleX(cVal);
				item.setScaleY(cVal);
				cursor.setScaleX(cVal);
				cursor.setScaleY(cVal);
			}
		});
	...
	private void scaleToNormal(final View item) {
		if (item == null) {
			return;
		}

		ObjectAnimator anim = ObjectAnimator//
				.ofFloat(item, "zhy", 1.05F, 1.0F)//
				.setDuration(100);//
		anim.start();
		anim.addUpdateListener(new AnimatorUpdateListener() {
			@Override
	...
	private void setBorderParams(final View item) {
		...
		case ANIM_DEFAULT:
			...
			if (scalable) {
				...
				item.setSelected(true);
				cursor.animate().scaleX(1.05f).scaleY(1.05f).setDuration(250)
						.start();
				// }
